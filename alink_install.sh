...
# Check if /home/radxa directory exists
  ...
  FILE_CONF="/etc/$FILE_NAME.conf"
    ...
    URL_ALINK_GS_CONF=$(github_asset_url "alink_gs.conf")

    if [ -z "$URL_ALINK_GS" ] || [ -z "$URL_ALINK_GS_CONF" ]; then
        echo_red "Error: Could not find required release files on GitHub."
        exit 1
    fi
    
    wget -O $FILE "$URL_ALINK_GS" && chmod +x $FILE
    wget -O "$CONF_FILE_PATH" "$URL_ALINK_GS_CONF"
...
[Service]
ExecStart=${FILE} --config ${CONF_FILE_PATH}
Type=idle
